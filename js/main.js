...
(function(){
    ...
    // ================== TTS (음성 재생) 기능 ==================
    let enVoice = null; let koVoice = null; let areVoicesLoaded = false;
    function loadVoices() {
    ...
    function startTimer(sec) {
        ...
        timerChip.style.display = 'inline-flex'; timerChip.textContent = `⏱ ${timeLeft}s`;
        timer = setInterval(() => {
    ...
    function goHome() {
        window.speechSynthesis.cancel(); isReplaying = false;
        setView('#view-sets'); crumb.textContent = '세트 선택'; 
        homeBtn.style.display = 'none'; ttsBtn.style.display = 'none';
        stopTimer(); 
    }
    ...
    function selectMainSet(setKey) {
        ...
        for (let i = 0; i < numSubsets; i++) {
            ...
            btn.addEventListener('click', () => { startSubSet(i); });
        ...
        allBtn.addEventListener('click', () => { startSubSet(-1); });
    ...
    function startSubSet(subSetIndex) {
        ...
        let setLabel = '';
        if (subSetIndex === -1) {
            currentWordList = mainSet; setLabel = `${labelForSet(currentSetKey)} · ALL Sets`;
        } else {
        ...
        chosenSetChip.textContent = setLabel; chosenSetModeChip.textContent = setLabel;
        setView('#view-levels');
    }
    
    function startLevel(lv) {
        currentLevel = lv; testMode = (lv === 4);
        if (lv === 3) { setView('#view-level3-mode'); } 
        else {
            quizDirection = 'en-ko';
            if (lv === 4) { idInput.value = ''; idModal.classList.remove('hidden'); } 
            else { startLevelCore(); }
        }
    }

    function startLevelCore() {
        queue = shuffle(currentWordList); solvedCount = 0; current = null;
        testDone = 0; testCorrect = 0; testWrongAnswers = [];
        ...
        ttsBtn.style.display = 'inline-flex'; setView('#view-quiz');
        nextQuestion();
    }
    ...
    function updateStatus() {
        ...
        if (testMode) { statusChip.textContent = `${labelForSet(currentSetKey)} · ${subsetLabel} · 진행 ${testDone}/${total} · 정답 ${testCorrect}`; } 
        else { statusChip.textContent = `${labelForSet(currentSetKey)} · ${subsetLabel} · 완료 ${solvedCount}/${total}`; }
    }
    ...
    function showEndLevelView() {
        ...
        setView('#view-end-level'); ttsBtn.style.display = 'none';
        crumb.textContent = '단계 완료';
    }

    function nextQuestion() {
        if ((!testMode && solvedCount >= currentWordList.length) || (testMode && testDone >= currentWordList.length)) {
            if (testMode) sendResult({type:'summary', userId:testUserId, setKey:currentSetKey, setLabel:labelForSet(currentSetKey), level:4, total:currentWordList.length, correct:testCorrect, timestamp:new Date().toISOString(), userAgent:navigator.userAgent});
            showEndLevelView(); return;
        }
        if (queue.length === 0 && !testMode) { showEndLevelView(); return; }
        controlsDock.style.display = 'flex'; idkBtn.disabled = false; ttsBtn.disabled = false;
        current = queue.shift(); updateStatus();
    ...
    async function onChoice(e) {
        ...
        idkBtn.disabled = true; stopTimer();
        const elapsedSec = Math.round((performance.now() - qStart) / 100) / 10;
        if (correct) {
            playCorrectSound(); btn.classList.add('correct');
            if (testMode) { testDone++; testCorrect++; sendResult({type:'result', userId:testUserId, setKey:currentSetKey, setLabel:labelForSet(currentSetKey), level:4, word:current.en, correctKo:current.ko, selectedKo:btn.textContent, correct:true, elapsedSec, timestamp:new Date().toISOString(), userAgent:navigator.userAgent});
            } else { solvedCount++; }
            updateStatus(); await sleep(800); nextQuestion();
        } else {
            playWrongSound(); btn.classList.add('wrong');
            [...choicesEl.children].forEach(c => { if (c.dataset.correct === 'true') c.classList.add('correct'); });
            if (testMode) { testDone++; testWrongAnswers.push(current); sendResult({type:'result', userId:testUserId, setKey:currentSetKey, setLabel:labelForSet(currentSetKey), level:4, word:current.en, correctKo:current.ko, selectedKo:btn.textContent, correct:false, elapsedSec, timestamp:new Date().toISOString(), userAgent:navigator.userAgent});
            } else { queue.push(current); }
            updateStatus(); wrongPopup();
        }
    }
    ...
    function onTimeout() {
        ...
        const elapsedSec = 6;
        testDone++; testWrongAnswers.push(current);
    ...
    function wrongPopup() {
        ...
        if (data && data.example && data.translation && data.mnemonic) {
            detailHTML += `<div class="review-content"><p><strong>예문:</strong> ${data.example}</p><p><strong>해석:</strong> ${data.translation}</p><p><strong>암기법:</strong> ${data.mnemonic}</p></div>`;
        }
        modalDetail.innerHTML = detailHTML; modal.classList.remove('hidden');
    }

    function showReview() {
        setView('#view-review'); crumb.textContent = '오답 학습';
        ...
        if (testWrongAnswers.length === 0) {
            ...
            uniqueWrongAnswers.forEach(word => {
                ...
                if (data) {
                    card.innerHTML = `<h3>${word.en}<span>${word.ko}</span></h3><p><strong>예문:</strong> ${data.example}</p><p><strong>해석:</strong> ${data.translation}</p><p><strong>암기법:</strong> ${data.mnemonic}</p>`;
                } else {
    ...
    async function replayWrongAnswers() {
        ...
        isReplaying = true; replayWrongBtn.disabled = true; replayWrongBtn.textContent = '듣는 중...';
        const uniqueWrongAnswers = [...new Map(testWrongAnswers.map(item => [item['en'], item])).values()];
        for (const word of uniqueWrongAnswers) {
            if (!isReplaying) break;
            await speak(word.en, 'en-US', 0.9); await sleep(150);
            await speak(word.ko, 'ko-KR', 0.95); await sleep(500);
        }
        replayWrongBtn.disabled = false; replayWrongBtn.textContent = '오답 다시 듣기';
        isReplaying = false;
    }
    ...
    function setupEventListeners() {
        ...
        $$('#view-level3-mode [data-direction]').forEach(b => {
            b.addEventListener('click', () => { quizDirection = b.dataset.direction; startLevelCore(); });
        });
        idStart.addEventListener('click', () => {
            const v = (idInput.value || '').trim(); if (!v) { idInput.focus(); return; }
            testUserId = v; idModal.classList.add('hidden'); startLevelCore();
        });
        idCancel.addEventListener('click', () => { idModal.classList.add('hidden'); setView('#view-levels'); });
        homeBtn.addEventListener('click', goHome);
        reviewHomeBtn.addEventListener('click', goHome);
        ttsBtn.addEventListener('click', () => { if (current && quizDirection === 'en-ko') speak(current.en, 'en-US'); });
        ...
        idkBtn.addEventListener('click', () => {
            playWrongSound(); choicesEl.classList.remove('hidden');
            [...choicesEl.children].forEach(c => { c.disabled = true; if (c.dataset.correct === 'true') c.classList.add('correct'); });
            stopTimer();
            if (testMode) { onTimeout(); } 
            else { queue.push(current); updateStatus(); wrongPopup(); }
        });
        ...
        modalNext.addEventListener('click', async () => { modal.classList.add('hidden'); await sleep(100); nextQuestion(); });
